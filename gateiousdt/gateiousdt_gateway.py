...
class GateioUsdtRestApi(RestClient):
    ...
    def query_order(self):
        """ """
        for vt_symbol in self.gateway.query_contracts:
    ...
    def cancel_order(self, req: CancelRequest):
        ...
        self.add_request(
            method="DELETE", path=f"/api/v4/futures/usdt/orders/{gateway_id}", callback=self.on_cancel_order, on_failed=self.on_cancel_order_failed, extra=req
        )
    # ----------------------------------------------------------------------------------------------------
